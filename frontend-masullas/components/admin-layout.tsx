...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  // Admin navigation with translations - memoized to prevent re-renders
  const adminNavigation = useMemo(() => [
    { name: t('admin.layout.dashboard'), href: "/admin/dashboard", icon: LayoutDashboard },
    { name: t('admin.layout.users'), href: "/admin/dashboard/users", icon: Users },
    { name: t('admin.layout.bookings'), href: "/admin/dashboard/bookings", icon: CalendarDays },
    { name: t('admin.layout.campsites'), href: "/admin/dashboard/campsites", icon: Tent },
    { name: t('admin.layout.devices'), href: "/admin/dashboard/devices", icon: Cpu },
    { name: t('admin.layout.settings'), href: "/admin/dashboard/settings", icon: Settings }
  ], [t])
      ...
      >
        <div className="flex h-16 items-center border-b border-border/20 px-4">
          ...
          >
            ...
            <span className="sr-only">{t('admin.layout.toggleSidebar')}</span>
          </Button>
        </div>
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-9 w-9 rounded-full" size="icon">
                  <Avatar className="h-9 w-9 border-2 border-secondary/10">
                    <AvatarImage
                      src={`https://api.dicebear.com/7.x/initials/svg?seed=${user ? `${user.firstName} ${user.lastName}` : t('admin.layout.admin')}`}
                      alt={t('admin.layout.admin')}
                    />
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                <DropdownMenuLabel>
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-medium leading-none">{user ? `${user.firstName} ${user.lastName}` : t('admin.layout.admin')}</p>
