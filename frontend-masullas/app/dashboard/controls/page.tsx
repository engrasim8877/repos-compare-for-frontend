...
const TrafficBarrier = ({ isOpen, isAnimating }: { isOpen: boolean; isAnimating: boolean }) => {
  const pivotX = 10;
  const pivotY = 90;
  const armWidth = 80;
  const armHeight = 10;
...
export default function CampsiteControlsPage() {
  ...
  const toggleUtility = async (campsiteId: string, utilityType: string, currentState: boolean) => {
    ...
    try {
      ...
      // Command successful
      toast({
        title: t('dashboard.controls.commandSent'),
        description: `${utilityType.charAt(0).toUpperCase() + utilityType.slice(1)} ${t('dashboard.controls.commandSuccessful')}`,
      });
      ...
      toast({ title: t('dashboard.controls.error'), description: err.message, variant: "destructive" });
        ...
        <DashboardLayout>
          <div className="flex flex-col gap-6 max-w-xl mx-auto py-8">
            <div className="flex flex-col gap-2">
              <h1 className="text-2xl font-bold tracking-tight">{t('dashboard.controls.title')}</h1>
              <p className="text-muted-foreground">{t('dashboard.controls.subtitle')}</p>
            </div>
            <Card className="shadow-sm border-yellow-200 bg-yellow-50/50">
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-xl font-semibold mb-2">{t('dashboard.controls.noActiveBookingTitle')}</h3>
                <p className="text-muted-foreground mb-6 text-center max-w-md">
                  {t('dashboard.controls.noActiveBookingMessage')}
                </p>
                <Link href="/dashboard/bookings">
                  <Button variant="default">
                    <BadgePlus className="mr-2 h-4 w-4" />
                    {t('dashboard.controls.requestBooking')}
                  </Button>
                </Link>
              </CardContent>
            </Card>
        ...
        <DashboardLayout>
          <div className="flex flex-col gap-6 max-w-xl mx-auto py-8">
            ...
            <Alert variant="destructive">
              ...
              <AlertTitle>{t('dashboard.controls.errorLoadingControls')}</AlertTitle>
              <AlertDescription>{error}</AlertDescription>
            </Alert>
            <Button className="w-fit" onClick={fetchUserProfile}>
              <RefreshCw className="mr-2 h-4 w-4" />
              {t('dashboard.controls.retryLoadingProfile')}
            </Button>
          </div>
        </DashboardLayout>
      </AuthGuard>
        ...
        <div className="flex flex-col gap-6 max-w-xl mx-auto py-8">
          ...
          <div className="flex flex-col gap-4 mb-4">
            <div className="flex justify-between items-center">
              ...
              >
                {deviceOffline ? (
                  <><WifiOff className="h-3 w-3 mr-1.5" /> {t('dashboard.controls.offline')}</>
                ) : (
                  <><Wifi className="h-3 w-3 mr-1.5" /> {t('dashboard.controls.online')}</>
                )}
              </Badge>
            </div>
            <div className="flex justify-between items-center">
              <p className="text-muted-foreground text-sm">{t('dashboard.controls.subtitle')}</p>
              <div className="flex items-center gap-2">
                <CampsiteSelector 
                ...
                <Button 
                  ...
                  aria-label={t('dashboard.controls.refreshControls')}
                >
                  {isRefreshing ? (
             ...
             {error && profileData && (
                <Alert variant="destructive" className="mt-2">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertTitle>{t('dashboard.controls.stateUpdateError')}</AlertTitle>
                  <AlertDescription>{error} - {t('dashboard.controls.stateUpdateErrorMessage')}</AlertDescription>
                </Alert>
             )}
          </div>
              ...
              <CardContent className="p-5 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold">{t('dashboard.controls.electricity')}</h3>
                      <Badge 
                        variant="outline"
                      ...
                      >
                        {currentElectricityState ? t('dashboard.controls.on') : t('dashboard.controls.off')}
                      </Badge>
                    </div>
                  </div>
                  ...
                  {isUpdating['electricity'] ? (
                    ...
                    <Switch 
                      ...
                      aria-label={t('dashboard.controls.toggleElectricity')}
                    />
                  )}
                </div>
              </CardContent>
              ...
              <CardContent className="p-5 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold">{t('dashboard.controls.waterSupply')}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        >
                          {currentWaterState ? t('dashboard.controls.on') : t('dashboard.controls.off')}
                        </Badge>
                        <Badge variant="secondary" className="text-xs font-normal h-5">{t('dashboard.controls.readOnly')}</Badge>
                      </div>
                    </div>
                  </div>
              ...
              <CardContent className="p-5 md:p-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold">{t('dashboard.controls.gateBarrier')}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge 
                           variant="outline"
                        ...
                        >
                          {currentBarrierState ? t('dashboard.controls.open') : t('dashboard.controls.closed')}
                        </Badge>
                        {barrierAnimation !== "idle" && (
                          <Badge variant="outline" className="animate-pulse text-xs h-5">
                            {barrierAnimation === "opening" ? t('dashboard.controls.opening') : t('dashboard.controls.closing')}
                          </Badge>
                        )}
                      </div>
                    </div>
                ...
                <Button 
                  ...
                  aria-label={currentBarrierState ? t('dashboard.controls.closeBarrierLabel') : t('dashboard.controls.openBarrierLabel')}
                >
                                      {isUpdating['barrier'] ? (
                      <Loader2 className="h-5 w-5 animate-spin" />
                    ) : (
                      currentBarrierState ? t('dashboard.controls.closeBarrier') : t('dashboard.controls.openBarrier')
                    )}
                </Button>
              </CardContent>
            </Card>
