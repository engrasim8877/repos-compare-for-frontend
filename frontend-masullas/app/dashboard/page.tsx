            ...
            <Card className="w-full max-w-md shadow-xl">
              ...
              <CardContent className="space-y-4 text-center">
                ...
                <div className="text-sm text-muted-foreground">
                  <p>{t('dashboard.afterBookingApproval')}</p>
                  <ul className="mt-2 space-y-1">
                    <li className="flex items-center gap-2">
                      <Info className="h-4 w-4 text-primary" />
                      <span>{t('dashboard.controlUtilities')}</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <Info className="h-4 w-4 text-primary" />
                      <span>{t('dashboard.monitorUsage')}</span>
                    </li>
                    <li className="flex items-center gap-2">
                      <Info className="h-4 w-4 text-primary" />
                      <span>{t('dashboard.accessFeatures')}</span>
                    </li>
                  </ul>
                </div>
              </CardContent>
              <CardFooter className="flex justify-center">
                <Button size="lg" asChild>
                  <Link href="/dashboard/bookings">{t('dashboard.requestFirstBooking')}</Link>
                </Button>
              </CardFooter>
            </Card>
          </div>
    ...
    return (
      <div className="flex flex-col gap-6 py-8">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold tracking-tight">{t('dashboard.pageTitles.dashboard')}</h1>
          <p className="text-muted-foreground">{t('dashboard.errorLoadingDashboard')}.</p>
        </div>
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>{t('dashboard.errorLoadingDashboard')}</AlertTitle>
  ...
  return (
    <div className="flex flex-col gap-8 py-6"> {/* Increased gap and added padding */}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">{t('dashboard.welcomeBack')}, {user?.firstName}!</h1>
                ...
                <div className="flex-1">
                  ...
                  <div className="mb-1.5 flex items-center text-sm bg-green-50 text-green-700 px-2.5 py-1 rounded-md w-fit">
                    <span className="font-medium">
                      {calculateDaysRemaining(activeBooking.endDate)} {t('dashboard.daysRemaining')}
                    </span>
                  </div>
                  {/* Device Status */} 
                  <div className={`flex items-center text-xs font-medium rounded-full px-2.5 py-0.5 w-fit ${isDeviceOffline ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-200'}`}>
                    ...
                    {isDeviceOffline ? t('dashboard.deviceOffline') : t('dashboard.deviceOnline')}
                  </div>
                </div>
            ...
            <div className="relative">
              ...
              <div className="p-5 md:p-6 flex flex-col sm:flex-row items-start gap-5">
                <div className="flex-1">
                  ...
                  <div className="flex items-center text-sm text-muted-foreground mb-2.5">
                    <CalendarDays className="h-3.5 w-3.5 mr-1.5 text-muted-foreground/70" />
                    {t('dashboard.from')} {formatDate(approvedBooking.startDate)}
                  </div>
                  <div className="flex items-center text-sm bg-blue-50 text-blue-700 px-2.5 py-1 rounded-md w-fit">
                    <span className="font-medium">
                      {t('dashboard.startsIn')} {calculateDaysUntil(approvedBooking.startDate)} {t('dashboard.days')}
                    </span>
                  </div>
                </div>
                <div className="w-full sm:w-auto self-stretch sm:self-center flex mt-4 sm:mt-0">
                  <Link href={`/dashboard/bookings`} passHref className="w-full sm:w-auto">
                    <Button variant="outline" className="w-full border-blue-200 text-blue-700 hover:bg-blue-50 hover:text-blue-800">
                      {t('dashboard.viewBookingDetails')}
                      <ArrowUpRight className="ml-1.5 h-4 w-4" />
                    </Button>
                  </Link>
                </div>
            ...
            <div className="relative">
              ...
              <div className="p-5 md:p-6 flex flex-col sm:flex-row items-start gap-5">
                <div className="flex-1">
                  ...
                  <h2 className="text-xl font-semibold mb-1.5">{t('dashboard.bookingRequest')}</h2>
                  <div className="flex items-center text-sm text-muted-foreground">
                    <span className="font-medium text-muted-foreground/80 mr-1.5">{t('dashboard.campsite')}:</span>
            ...
            <div className="relative">
              ...
              <div className="p-5 md:p-6 flex flex-col sm:flex-row items-start gap-5">
                <div className="flex-1">
                  ...
                  <h2 className="text-xl font-semibold mb-1.5">{t('dashboard.requestFirstBooking')}</h2>
                  <p className="text-sm text-muted-foreground mb-2.5">
                    {t('dashboard.firstBookingMessage')}.
                  </p>
                </div>
                <div className="w-full sm:w-auto self-stretch sm:self-center flex mt-4 sm:mt-0">
                  <Link href="/dashboard/bookings" passHref className="w-full sm:w-auto">
              ...
              <CardContent className="p-5 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold">{t('dashboard.electricity')}</h3>
                      <Badge 
                        variant="outline"
                      ...
                      >
                        {activeCampsite.currentState?.electricity ? t('dashboard.on') : t('dashboard.off')}
                      </Badge>
                    </div>
                  </div>
                  <Switch 
              ...
              <CardContent className="p-5 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold">{t('dashboard.waterSupply')}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        >
                                                      {activeCampsite.currentState?.water ? t('dashboard.on') : t('dashboard.off')}
                        </Badge>
                        <Badge variant="secondary" className="text-xs font-normal h-5">{t('dashboard.readOnly')}</Badge>
                      </div>
                    </div>
                  </div>
              ...
              <CardContent className="p-5 md:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold">{t('dashboard.gateBarrier')}</h3>
                      <Badge 
                        variant="outline"
                        className={`mt-1 text-xs px-2 py-0.5 rounded-full ${
                          activeCampsite.currentState?.barrier
                      ...
                      >
                        {activeCampsite.currentState?.barrier ? t('dashboard.open') : t('dashboard.closed')}
                      </Badge>
                    </div>
                  </div>
                  <Switch 
