    ...
    try {
      ...
      if (!data.hasActiveBooking) {
        toast({
          ...
          description: "You don't have an active campsite booking for today.",
          variant: "destructive",
        })
      } else if (!data.hasCampsite) {
        toast({
          title: "No Campsite Assigned",
          description: "Your booking doesn't have a campsite assigned yet.",
          variant: "destructive",
        })
      }
    } catch (err: any) {
      ...
      toast({
        title: "Error",
        description: err.message || "Failed to load campsite data",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-semibold">
                {summaryData.device.status.toUpperCase()}
              </span>
            </div>
            <p className="text-sm text-muted-foreground mt-2">
              Last connected: {format(parseISO(summaryData.device.lastConnected), "h:mm a")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold mb-2">
              {calculateElectricityUsage(allHistory).toFixed(6)} kWh
            </div>
            <p className="text-sm text-muted-foreground">
  ...
  const getCommandDescription = (historyItem: HistoryItem): { title: string; details: string[] } => {
    if (historyItem.type === "command") {
      ...
      if (commandType === "setElectricity") return {
        ...
        details: [`Changed state to ${state}`]
      };
      ...
      if (commandType === "setBarrier") return {
        ...
        details: [`${state === "ON" ? "Opened" : "Closed"} the barrier`]
      };
      ...
      if (newState.electricity !== undefined) {
        changes.push(`Electricity: ${newState.electricity ? "ON" : "OFF"}`);
      }
      if (newState.water !== undefined) {
        changes.push(`Water: ${newState.water ? "ON" : "OFF"}`);
      }
      if (newState.barrier !== undefined) {
        changes.push(`Barrier: ${newState.barrier ? "OPEN" : "CLOSED"}`);
      }
      
      return {
        title: "State Update",
        details: changes.length > 0 ? changes : ["State values updated"]
      };
    }
  }
      ...
      return (
        <Card>
          <CardContent className="p-6 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-medium mb-1">No Command History</h3>
            <p className="text-muted-foreground max-w-md">
              There is no command or state change history recorded for your active booking period.
            </p>
          </CardContent>
        </Card>
      );
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-xs">
              {displayedHistory.length} of {allHistory.length} records
            </Badge>
          </div>
          <CardDescription>
            Showing most recent commands and state changes
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <div className="flex flex-col gap-6 container mx-auto p-4 lg:p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-muted-foreground">
                {summaryData?.booking 
                  ? `Viewing data for ${formatDate(summaryData.booking.startDate)}`
                  : "View your electricity usage and history"}
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <Alert variant="warning">
              ...
              <AlertDescription>
                You don't have an active campsite booking for today.
              </AlertDescription>
            </Alert>
          ) : !summaryData?.hasCampsite ? (
            <Alert variant="warning">
              <AlertTriangle className="h-4 w-4" />
              <AlertTitle>No Campsite Assigned</AlertTitle>
              <AlertDescription>
                Your booking doesn't have a campsite assigned yet.
              </AlertDescription>
            </Alert>
          ) : (
            <>
